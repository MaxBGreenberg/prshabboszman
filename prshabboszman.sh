...
# Certain partsdepends on my form of hebcal
...
hebcal -ZC "${CITY}" $FRI | grep -A 2 Sunset >> /tmp/psz.tmp			# Prints sunset time, bein hashmashos, tzeis hakochavim, and chatzos halayla for Leil Shabbos to same file
hebcal --mevarchim -ZC "${CITY}" $SAT >> /tmp/psz.tmp				# Prints zmanim for Shabbos day and havdala time to same file
